...
Config = {
    ...
    "SYMBOL": 'BTCUSDT',
    "INTERVALS": ['1h', '4h', '1d'],  # Use a list of intervals
    "HIST_POINTS": 360,
    "PRED_HORIZON": 24,
    "N_PREDICTIONS": 30,
    "VOL_WINDOW": 24,
...
def make_prediction(df, predictor, interval):
    ...
    start_new_range = last_timestamp + pd.Timedelta(hours=1 if interval == '1h' else (4 if interval == '4h' else 24))
...
def fetch_binance_data(interval):
...
def create_plot(hist_df, close_preds_df, volume_preds_df, interval):
    """Generates and saves a comprehensive forecast chart."""
    print(f"Generating comprehensive forecast chart for {interval}...")
    ...
    if interval == '1h':
        ...
        timedelta_val = timedelta(hours=4)
    else: # 1d
        timedelta_val = timedelta(days=1)

    pred_time = pd.to_datetime([last_hist_time + timedelta_val * (i + 1) for i in range(len(close_preds_df))])
    ...
    ax1.set_title(f'{Config["SYMBOL"]} Probabilistic Price & Volume Forecast (Next {Config["PRED_HORIZON"]} {interval} intervals)', fontsize=16, weight='bold')
    ...
    separator_time = hist_time.iloc[-1] + timedelta_val / 2
    for ax in [ax1, ax2]:
        ax.axvline(x=separator_time, color='red', linestyle='--', linewidth=1.5, label='_nolegend_')
        ax.tick_params(axis='x', rotation=30)

    fig.tight_layout()
    chart_path = Config["REPO_PATH"] / f'prediction_chart_{interval}.png'
...
def update_html(metrics):
    ...
    # Update metrics for each interval
    for interval, (upside_prob, vol_amp_prob) in metrics.items():
        upside_prob_str = f'{upside_prob:.1%}'
        vol_amp_prob_str = f'{vol_amp_prob:.1%}'

        # Correctly format the regex with the interval
        upside_regex = re.compile(fr'(<p class="metric-value" id="upside-prob-{interval}">).*?(</p>)')
        vol_amp_regex = re.compile(fr'(<p class="metric-value" id="vol-amp-prob-{interval}">).*?(</p>)')

        content = upside_regex.sub(
            lambda m: f'{m.group(1)}{upside_prob_str}{m.group(2)}',
            content
        )
        content = vol_amp_regex.sub(
...
def main_task(model):
    ...
    metrics = {}  # Store metrics for all intervals
    
    for interval in Config["INTERVALS"]:
        print(f"\nProcessing {interval} interval...")
        df_full = fetch_binance_data(interval)
        df_for_model = df_full.iloc[:-1]

        close_preds, volume_preds, v_close_preds = make_prediction(df_for_model, model, interval)
        ...
        metrics[interval] = (upside_prob, vol_amp_prob)
        
        create_plot(hist_df_for_plot, close_preds, volume_preds, interval)

        # Clean up large objects for this interval
        del df_full, df_for_model, close_preds, volume_preds, v_close_preds
        del hist_df_for_plot, hist_df_for_metrics
        gc.collect()

    # Update HTML with all metrics
    update_html(metrics)
